...
`scripts/prepare_mos_worktree.py` reads only Git-tracked files from the
configured maintained-source worktree without modifying it, copies them into
ignored `worktree/`, then applies validated, drift-checked initial-port edits.
`include/ez80.h` is an audited facade over
