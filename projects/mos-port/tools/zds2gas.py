...
def translate(
    ...
    locations: Sequence[SourceLocation] | None = None,
    input_commit: str = "unversioned-single-file",
) -> Translation:
    physical_lines = text.splitlines(keepends=True)
    ...
    output: list[str] = [
        ...
        f"; agon-mos input commit: {input_commit}\n",
...
def translate_tree(source_root: Path, output_root: Path, check: bool) -> dict[str, object]:
    ...
    baseline_path = Path(__file__).resolve().parents[3] / "evidence" / "baseline.json"
    baseline = json.loads(baseline_path.read_text(encoding="utf-8"))
    input_commit = str(baseline["repositories"]["agon_mos"]["commit"])
