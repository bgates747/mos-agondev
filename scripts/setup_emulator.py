#!/usr/bin/env python3
"""Provision the project-local Fab profile and mandatory direct launcher."""
...
GENERATED_LAUNCHER_MARKER = "# Generated by mos-agondev setup_emulator.py."
...
def _require_generated_launcher_or_link(path: Path) -> None:
    """Accept the old managed link or a launcher generated by this project."""

    if path.is_symlink() or not path.exists():
        return
    if not path.is_file():
        raise SetupError(f"Refusing non-file profile launcher: {path}")
    try:
        contents = path.read_text(encoding="utf-8")
    except (OSError, UnicodeError) as error:
        raise SetupError(f"Could not inspect existing profile launcher: {path}") from error
    if GENERATED_LAUNCHER_MARKER not in contents:
        raise SetupError(f"Refusing to replace non-generated profile launcher: {path}")
...
def preflight_profile(profile: Path, inputs: RuntimeInputs) -> tuple[Path, ...]:
    ...
    _require_generated_launcher_or_link(profile / "fab-agon-emulator")
    for path, label in (
        (profile / "fab-agon-emulator.bin", "raw Fab executable link"),
...
def generated_launcher_text() -> str:
    """Return the canonical direct-entry launcher for this project profile."""

    return f'''#!/usr/bin/env bash
{GENERATED_LAUNCHER_MARKER}
set -euo pipefail

profile_dir="$(cd "$(dirname "${{BASH_SOURCE[0]}}")" && pwd -P)"
project_root="$(cd "${{profile_dir}}/.." && pwd -P)"
fab_bin="$(readlink -f "${{profile_dir}}/fab-agon-emulator.bin")"
candidate_mos="${{project_root}}/projects/mos-port/bin/MOS.bin"
sdl_lib="${{HOME}}/.local/lib"
...
for emulator_arg in "$@"; do
    case "${{emulator_arg}}" in
        ...
        --renderer|--renderer=*|--verbose|-z|--zero)
            printf 'fab-agon-emulator manages this option: %s\\n' \\
                "${{emulator_arg}}" >&2
            exit 2
            ;;
    esac
...
for profile_input in \\
    "${{fab_bin}}" "${{profile_dir}}/firmware" \\
    "${{candidate_mos}}" "${{profile_dir}}/sdcard"; do
    if [[ ! -e "${{profile_input}}" ]]; then
        printf 'Missing candidate MOS emulator input: %s\\n' "${{profile_input}}" >&2
        exit 1
    fi
done
if [[ ! -x "${{fab_bin}}" ]]; then
    printf 'Raw Fab executable is not executable: %s\\n' "${{fab_bin}}" >&2
    exit 1
fi
if [[ -f "${{sdl_lib}}/libSDL3.so.0" ]]; then
    export LD_LIBRARY_PATH="${{sdl_lib}}${{LD_LIBRARY_PATH:+:${{LD_LIBRARY_PATH}}}}"
fi
...
exec "${{fab_bin}}" \\
    ...
    --firmware platform \\
    --mos "${{candidate_mos}}" \\
    --sdcard "${{profile_dir}}/sdcard" \\
    --verbose -z \\
    "$@"
'''


def write_profile_launcher(profile: Path) -> None:
    launcher = profile / "fab-agon-emulator"
    _require_generated_launcher_or_link(launcher)
    if launcher.is_symlink():
        launcher.unlink()
    launcher.write_text(generated_launcher_text(), encoding="utf-8")
...
def setup_profile(profile: Path, fab_root: Path) -> Path:
    ...
    replace_symlink(profile / "fab-agon-emulator.bin", inputs.executable)
    write_profile_launcher(profile)
